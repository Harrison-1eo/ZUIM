...
void LFSRWithInitialisationMode(ZUC_State* state, u32 u)
{
    u32 f, v;
    f = state->LFSR_S0;
...
/* LFSR with work mode */
void LFSRWithWorkMode(ZUC_State* state)
{
    u32 f, v;
    f = state->LFSR_S0;
    v = MulByPow2(state->LFSR_S0, 8);
...
/* BitReorganization */
void BitReorganization(ZUC_State* state)
{
...
u32 F(ZUC_State* state)
{
    ...
    u = L1((W1 << 16) | (W2 >> 16));
    v = L2((W2 << 16) | (W1 >> 16));
    state->F_R1 = MAKEU32(S0[u >> 24], S1[(u >> 16) & 0xFF],
            S0[(u >> 8) & 0xFF], S1[u & 0xFF]);
    state->F_R2 = MAKEU32(S0[v >> 24], S1[(v >> 16) & 0xFF],
...
void ZUC_Next(ZUC_State* state, u32* pKeystream, int KeystreamLen)
{
    ...
    {
        BitReorganization(state);
        pKeystream[i] = F(state) ^ state->BRC_X3;
        LFSRWithWorkMode(state);
    }
}
