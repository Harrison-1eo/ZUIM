...
void ZUC256_MK_AVX512_Setup()
{
	...
	mbp_mask = _mm512_set1_epi32(MBP_MASK);
}

inline __m512i sbox0(const __m512i in)
{
...
/* ============== 矩阵乘法优化 ============== */
inline __m512i sbox1(const __m512i in)
{
...
#define V_FEEDBACK(f, v)                    \
	v = MBP2(LFSR_S[buf], 8);               \
	f = _mm512_add_epi32(LFSR_S[buf], v);   \
	f = AMR(f);                             \
	v = MBP2(LFSR_S[(4 + buf) & 0xF], 20);  \
	f = _mm512_add_epi32(f, v);             \
	f = AMR(f);                             \
	v = MBP2(LFSR_S[(10 + buf) & 0xF], 21); \
	f = _mm512_add_epi32(f, v);             \
	f = AMR(f);                             \
	v = MBP2(LFSR_S[(13 + buf) & 0xF], 17); \
	f = _mm512_add_epi32(f, v);             \
	f = AMR(f);                             \
	v = MBP2(LFSR_S[(15 + buf) & 0xF], 15); \
	f = _mm512_add_epi32(f, v);             \
	f = AMR(f)
...
inline void ZUC256_MK_AVX512_LFSRINIT(ZUC256_MK_AVX512_State *state, const u8 *k, const u8 *iv, const u8 *d)
{
...
void ZUC256_MK_AVX512_Init(ZUC256_MK_AVX512_State *state, u32 *ks, int wordlen, const u8 *k, const u8 *iv)
{
	...
	ZUC256_MK_AVX512_LFSRINIT(state, k, iv, EK_d);
...
void ZUC256_MK_AVX512_Gene(ZUC256_MK_AVX512_State *state, u32 *keystream, u32 keylen)
{
	...
	for (i = 0; i < keylen; i++)
	{
		...
		V_BITR();
		V_FSM();
		v = _mm512_xor_si512(W, BRC_X[3]);
		_mm512_storeu_si512(keystream + 16 * i, v);
	}
}
...
void ZUC256_MK_AVX512_Mac(ZUC256_MK_AVX512_State *state, u32 *MAC, int MAC_BITLEN, const u32 *M, const u32 LENGTH)
{
	...
	u32 d_index = (MAC_BITLEN >> 6) << 4;
	u32 MAC_WORDLEN = MAC_BITLEN >> 5;
	u32 L = LENGTH + 2 * MAC_WORDLEN;
