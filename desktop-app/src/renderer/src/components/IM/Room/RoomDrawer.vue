...
<template>
  <el-drawer show-close drawer size="450px">
    ...
    <el-card style="max-width: 480px">
      ...
      <template #footer>
        <el-button type="primary" class="changedescription" @click="changedescrpition"
          >修改房间描述（未设置方法）</el-button
        >
      </template>
    </el-card>
    ...
    <el-card style="max-width: 480px">
      ...
      <el-row>
        <el-col
          ...
          :key="user.id"
          :span="12"
          style="display: flex; text-align: center; margin: 10px 0"
        >
          <el-avatar :src="getAvatarUrl(user.avatar)" :size="40"></el-avatar>
...
export default {
  ...
  methods: {
    ...
    addUserBox() {
      ElMessageBox.prompt('请输入邀请的用户名称', '邀请朋友')
        .then(({ value }) => {
          if (!value) {
    ...
    async fetchRoomUsers() {
      ...
      try {
        const response = await axios.get(
          'http://localhost:8000/api/room/members?room_id=' + this.roomID
        )
