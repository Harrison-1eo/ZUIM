...
JSMpeg.Player = function() {
    ...
    var Player = function(url, options) {
        ...
        if (this.wasmModule) {
            if (this.wasmModule.ready) { this.startLoading() } else if (JSMpeg.WASM_BINARY_INLINED) {
    ...
    Player.prototype.startLoading = function() { this.source.start(); if (this.autoplay) { this.play() } };
    ...
    Player.prototype.nextFrame = function() { if (this.source.established && this.video) { return this.video.decode() } return false };
    Player.prototype.updateForStaticFile = function() {
        ...
        if (notEnoughData && this.source.completed) { if (this.loop) { this.seek(0) } else { this.pause(); if (this.options.onEnded) { this.options.onEnded(this) } } } else if (notEnoughData && this.options.onStalled) { this.options.onStalled(this) }
    };
    return Player
}();
...
JSMpeg.Source.Fetch = function() {
    ...
    FetchSource.prototype.connect = function(destination) { this.destination = destination };
    ...
    FetchSource.prototype.pump = function(reader) { return reader.read().then(function(result) { if (result.done) { this.completed = true } else { if (this.aborted) { return reader.cancel() } if (this.destination) { this.destination.write(result.value.buffer) } return this.pump(reader) } }.bind(this)).catch(function(err) { throw err }) };
    FetchSource.prototype.resume = function(secondsHeadroom) {};
    FetchSource.prototype.abort = function() { this.aborted = true };
    return FetchSource
}();
JSMpeg.Source.AjaxProgressive = function() {
    ...
    AjaxProgressiveSource.prototype.loadNextChunk = function() {
        ...
        if (start >= this.fileSize || this.aborted) { this.completed = true; if (this.onCompletedCallback) { this.onCompletedCallback(this) } return }
        this.isLoading = true;
...
JSMpeg.Source.WebSocket = function() {
    ...
    WSSource.prototype.start = function() {
        ...
        this.progress = 0;
        this.established = false;
        if (this.options.protocols) { this.socket = new WebSocket(this.url, this.options.protocols) } else { this.socket = new WebSocket(this.url) }
...
JSMpeg.Decoder.Base = function() {
    ...
    BaseDecoder.prototype.bufferGetIndex = function() { return this.bits.index };
    BaseDecoder.prototype.bufferSetIndex = function(index) { this.bits.index = index };
    BaseDecoder.prototype.bufferWrite = function(buffers) { return this.bits.write(buffers) };
...
JSMpeg.Decoder.MPEG1VideoWASM = function() {
    ...
    MPEG1WASM.prototype.initializeWasmDecoder = function() {
        if (!this.module.instance) { console.warn("JSMpeg: WASM module not compiled yet"); return }
    ...
    MPEG1WASM.prototype.bufferGetIndex = function() { if (!this.decoder) { return } return this.functions._mpeg1_decoder_get_index(this.decoder) };
    ...
    MPEG1WASM.prototype.write = function(pts, buffers) { JSMpeg.Decoder.Base.prototype.write.call(this, pts, buffers); if (!this.hasSequenceHeader && this.functions._mpeg1_decoder_has_sequence_header(this.decoder)) { this.loadSequenceHeader() } };
...
JSMpeg.Decoder.MP2AudioWASM = function() {
    ...
    MP2WASM.prototype.bufferGetIndex = function() { if (!this.decoder) { return } return this.functions._mp2_decoder_get_index(this.decoder) };
    ...
    MP2WASM.prototype.getCurrentTime = function() { var enqueuedTime = this.destination ? this.destination.enqueuedTime : 0; return this.decodedTime - enqueuedTime };
...
JSMpeg.Renderer.WebGL = function() {
    ...
    WebGLRenderer.prototype.handleContextLost = function(ev) { ev.preventDefault() };
    WebGLRenderer.prototype.handleContextRestored = function(ev) { this.initGL() };
    WebGLRenderer.prototype.destroy = function() {
        ...
        if (this.ownsCanvasElement) { this.canvas.remove() }
    };
    ...
    WebGLRenderer.prototype.updateTexture = function(unit, texture, w, h, data) {
        ...
        gl.bindTexture(gl.TEXTURE_2D, texture);
        if (this.hasTextureData[unit]) { gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, w, h, gl.LUMINANCE, gl.UNSIGNED_BYTE, data) } else {
...
JSMpeg.Renderer.Canvas2D = function() {
    ...
    CanvasRenderer.prototype.destroy = function() { if (this.ownsCanvasElement) { this.canvas.remove() } };
...
JSMpeg.WASMModule = function() {
    ...
    WASM.prototype.write = function(buffer) { this.loadFromBuffer(buffer) };
    WASM.prototype.loadFromFile = function(url, callback) {
        if (callback) { this.onInitCallbacks.push(callback) }
        if (this.loadingFromFileStarted) { return }
    ...
    WASM.prototype.loadFromBuffer = function(buffer, callback) {
        if (callback) { this.onInitCallbacks.push(callback) }
        if (this.loadingFromBufferStarted) { return }
        this.loadingFromBufferStarted = true;
        this.moduleInfo = this.readDylinkSection(buffer);
        if (!this.moduleInfo) { for (var i = 0; i < this.onInitCallbacks.length; i++) { this.onInitCallbacks[i](null) } return }
        this.memory = new WebAssembly.Memory({ initial: 256 });
        var env = { memory: this.memory, memoryBase: 0, __memory_base: 0, table: new WebAssembly.Table({ initial: this.moduleInfo.tableSize, element: "anyfunc" }), tableBase: 0, __table_base: 0, abort: this.c_abort.bind(this), ___assert_fail: this.c_assertFail.bind(this), _sbrk: this.c_sbrk.bind(this) };
        ...
        WebAssembly.instantiate(buffer, { env: env }).then(function(results) {
            ...
            this.ready = true;
            for (var i = 0; i < this.onInitCallbacks.length; i++) { this.onInitCallbacks[i](this) }
        }.bind(this))
    };
    ...
    WASM.prototype.align = function(addr) { var a = Math.pow(2, this.moduleInfo.memoryAlignment); return Math.ceil(addr / a) * a };
    ...
    WASM.prototype.c_abort = function(size) { console.warn("JSMPeg: WASM abort", arguments) };
    WASM.prototype.c_assertFail = function(size) { console.warn("JSMPeg: WASM ___assert_fail", arguments) };
    WASM.prototype.readDylinkSection = function(buffer) {
        ...
        var matchNextBytes = function(expected) { for (var i = 0; i < expected.length; i++) { var b = typeof expected[i] === "string" ? expected[i].charCodeAt(0) : expected[i]; if (bytes[next++] !== b) { return false } } return true };
        if (!matchNextBytes([0, "a", "s", "m"])) { console.warn("JSMpeg: WASM header not found"); return null }
        var next = 9;
        var sectionSize = readVarUint();
        if (!matchNextBytes([6, "d", "y", "l", "i", "n", "k"])) { console.warn("JSMpeg: No dylink section found in WASM"); return null }
        return { memorySize: readVarUint(), memoryAlignment: readVarUint(), tableSize: readVarUint(), tableAlignment: readVarUint() }
    };
    WASM.IsSupported = function() { return !!window.WebAssembly };
    WASM.GetModule = function() { WASM.CACHED_MODULE = WASM.CACHED_MODULE || new WASM; return WASM.CACHED_MODULE };
    return WASM
}();
